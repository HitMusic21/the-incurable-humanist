...
**The Incurable Humanist (TIH2)** — a personal publication platform for Denise Rodriguez Dao.

### ⚠️ PRODUCTION RUNS ON CLOUDFLARE WORKERS — read this first

Most of the sections below describe the **legacy** Cloud Run stack (FastAPI +
Postgres). That stack no longer serves production. As of Sep 2026:

| | Production | Legacy (still in the repo) |
|---|---|---|
| Origin | **`worker/src/worker.py`** — FastAPI on Cloudflare Python Workers | `backend/app` |
| Database | **Cloudflare D1** (SQLite) | Postgres |
| Static assets | **Workers Static Assets** (`worker/public/`) | nginx container |
| Deploy | **`cd worker && uv run pywrangler deploy`** | Cloud Build → Cloud Run |
| Tests | **`cd worker && uv run pytest tests/`** (119) | `backend/tests` |

`backend/` is kept for its scripts (backfill, reconcile, image rehosting) and
as the reference implementation the Worker was ported from. **Do not assume a
change to `backend/app` affects the live site — it does not.**

See `docs/ARCHITECTURE.md` for the current system.
...
```bash
# one-shot: import the full archive (76 posts as of Sep 2026, ~2 min at 1.5s pacing)
...
## Conventions added Sep 2026

These sit alongside the UI conventions above and are enforced by tests.

### Sitemap is a build artifact

`sitemap.xml` is generated by `npm run build`, not served from D1. **An essay
synced by the hourly cron is live and linked but absent from the sitemap until
the next build.** That is precisely how the Sep 15 essay remained "unknown to
Google" while serving correctly. Rebuild and redeploy after a sync lands a new
essay.

### `max-image-preview:large` is required, not optional

Set in **both** `SEO.tsx` (runtime) and `prerender.mjs` (prerendered head).
It is the hard requirement for Google Discover — an image-led surface where a
thumbnail-only preview means no placement. `SEO.test.tsx` fails if it is
dropped. noindex pages must NOT carry it.

### One URL per page

Trailing-slash variants 301 to the bare form in `worker.py`. The redirect must
stay **after** the retired-route check, or `/archive/<slug>/` chains through
two hops instead of one and dilutes the link equity that alias exists to
preserve. Guarded by `test_trailing_slash_redirect_runs_after_retired_routes`.

### Prose is ragged-right and unhyphenated

Already covered above, but restated because it was previously *mandated the
other way*: do not re-add `text-justify` or `hyphens-auto`. Denise read
hyphenated line breaks as words being cut off.

### Third-party embeds go behind `ConsentFacade`

Spotify and YouTube both use it. An eager iframe sets vendor cookies on page
view, which is exactly what `ConsentBanner` exists to prevent. New embeds must
follow suit rather than rendering directly.

### Social icons are keyed off `SITE.socials`

`SocialIconRow.tsx` is typed as `Record<keyof typeof SITE.socials, …>`, so
adding a network to config without adding its icon **fails the build**. Do not
inline icons into Footer or Home again — that was the two-file tax this
replaced.
...
| Question | Doc |
|---|---|
| How does production actually work? | **`docs/ARCHITECTURE.md`** |
| Who reads this publication, and what grows it? | `docs/ICP_AND_GROWTH_ANALYSIS.md` |
| What should Denise write next? | `docs/ESSAY_TOPIC_RESEARCH_SEP2026.md` |
| What SEO work is still outstanding? | `docs/SEO_REMAINING_SETUP.md` |
| What changed in the UI and why? | `frontend/CHANGELOG.md` |
